...
$confirm_pass = sha1($_POST['confirmPassword']);
		...
		$qry="SELECT * FROM Donor WHERE pid=$pid AND password='$cur_pass'";
		//echo $qry;
		...
		if(mysql_num_rows($result) > 0) 
		{
					if($new_pass == $confirm_pass)
						{
						$updateQuery = "UPDATE Donor SET password='".$new_pass."' WHERE email='".$current_email."'";
						$result = mysql_query($updateQuery);
						if($result)
						{
						Header("refresh:0.001;url=http://localhost/sampark/NGO-portal-website/donorhome.php?id=".$pid);
						return true;
						}
					}
					else
