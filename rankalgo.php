...
*/
	...
	function onVote($Ngo_pid, $donor_pid, $rate)
	{
		...
		*/
		$total_rate=mysql_query("SUM(SELECT rating from rates WHERE Ngo_pid=$Ngo_pid)"); // R
		...
		$total_rate_all=mysql_query("SUM(Select rating from rates)");
		$total_votes_all=mysql_query("SELECT COUNT(ngo_pid, donor_pid) from rates");
		$C=$total_rate_all/$total_votes_all;
		...
		mysql_query("UPDATE Ngo SET rate=$R, weighted_rate=$WR WHERE pid=$Ngo_pid");
